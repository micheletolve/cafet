...
<div class="container">
    ...
    <blockquote class="blockquote"><i class="fa fa-quote-left fa-1x" aria-hidden="true"></i> Eliminazione definitiva stao di investimento <%=Html.Encode(ModelStatus.Stato) %>  <i class="fa fa-quote-right fa-1x" aria-hidden="true"></i></blockquote>
    ...
    <div class="row">
        <table class="table table-hover">
            <tr>
                <th class="detail"><label for="Stato" class="col-form-label">Stato investimento</label></th>
                <td class="detail"><%=Html.Encode(ModelStatus.Stato) %></td>
            </tr>            
        </table>
    </div>

    <form action="?controller=Status&action=ErasePost" method="POST" id="ErasePost">
